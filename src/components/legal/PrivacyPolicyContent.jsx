...
const PrivacyPolicyContent = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      let currentSection = "";
      sections.forEach((section) => {
        const element = document.getElementById(section.policy.title);
        if (element) {
          const rect = element.getBoundingClientRect();
          if (rect.top <= 100 && rect.bottom >= 100) {
            currentSection = section.policy.title;
          }
        }
      });
      setActiveSection(currentSection);
  ...
  const scrollToSection = (policyId) => {
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row gap-6 px-7 pt-5 pb-5 lg:px-[50px] lg:pt-10 lg:pb-10">
        ...
        <div className="hidden md:block md:w-1/4 lg:w-1/5">
          <div className="sticky top-20 bg-white p-7 shadow-md rounded-xl w-fit min-w-[180px]">
            <h4 className="font-avenir-black text-gray-700">
              TABLE OF CONTENTS
            </h4>
            <ul className="mt-4 space-y-2 list-none">
              {policy.map((policy) => (
                <li key={policy.policy_id}>
                  <button
                    onClick={() => scrollToSection(policy.policy_id)}
                    className={`block w-full text-left text-sm font-medium ${
                      activeSection === `policy-${policy.policy_id}`
                        ? "text-primary font-bold"
                        : "text-gray-600"
                    } hover:text-primary`}
                  >
        ...
        <div className="md:w-3/4 md:mr-5 lg:w-4/5 lg:mr-15">
          ...
          <div className="mt-5 lg:mt-10">
            At FullSuite (“Company”), we are committed to protecting the privacy
            and security of your personal information. This Data Privacy Policy
            outlines how we collect, use, store, dispose and protect your
            personal data. By accessing our website and/or submitting your
            information directly to the Company, you agree and consent to the
            use, collection, processing, storage, and disposal of your personal
            data as outlined in this Privacy Policy. The Company is committed to
